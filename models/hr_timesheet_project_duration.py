...
class project_duration_timesheet(models.Model):
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if ((emp_name == proj_duration_employee) and (number <= proj_duration_hours)) or self.env.user.has_group('hr.group_hr_manager'):
                return res
...
class project_duration_model(models.Model):
    ...
    proj_dur = fields.One2many('project.duration', 'proj_duration_id')
